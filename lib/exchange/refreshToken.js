...
module.exports = function(options, issue) {
  ...
  return function refresh_token(req, res, next) {
    ...
    function issued(err, accessToken, refreshToken, params, resFormatter) {
      ...
      if (resFormatter && typeof resFormatter == 'function') {
        // resFormatter receives express res as first arg and tok json as second arg
        // If a return is provided it will override the json response.
        var optionalResp = resFormatter(res, tok)
        if (optionalResp) { json = typeof myVar === 'string' ? optionalResp : JSON.stringify(optionalResp) }
      }
      res.end(json);
    }
